...
import io.qusay.services.data.model.Module;
import io.qusay.services.ga.geneticalgorithm.Gene;
import io.qusay.services.ga.geneticalgorithm.GeneticAlgorithmJobData;
import io.qusay.services.ga.geneticalgorithm.Timeslot;
import io.qusay.services.ga.geneticalgorithm.Venue;
import io.qusay.services.data.api.JobRepository;
import io.qusay.services.data.api.LecturerTimeslotPreferenceRepository;
import io.qusay.services.data.api.ModuleRepository;
import io.qusay.services.data.api.ScheduleRepository;
import io.qusay.services.data.api.ScheduledModuleRepository;
import io.qusay.services.data.api.TimeslotRepository;
import io.qusay.services.data.api.VenueRepository;
import io.qusay.services.data.model.CourseModule;
import io.qusay.services.data.model.DepartmentBuilding;
...
import io.qusay.services.data.model.ScheduledModule;
...
public class DbToGaDeserializer {
    ...
    private List<Timeslot> generateTimeslotsFromDatabase() {
        Iterable<io.qusay.services.data.model.Timeslot> entities = timeslotRepository.findAll();

        // Translate all those timeslots into GA Timeslots objects
        List<Timeslot> timeslots = new ArrayList<>();
        for (io.qusay.services.data.model.Timeslot entity : entities) {
    ...
    private List<Venue> generateVenuesFromDatabase() {
        Iterable<io.qusay.services.data.model.Venue> entities = venueRepository.findAll();

        // Translate all those venues into GA Venue objects
        List<Venue> venues = new ArrayList<>();
        for (io.qusay.services.data.model.Venue entity : entities) {
...
//            }          // DEBUG
            ...
            modules.add(new io.qusay.services.ga.geneticalgorithm.Module(entity.getModuleId(), entity.getName(), totalEnrolled, entity.getLab(),
    ...
    private List<Gene> generateScheduledModulesFromDatabase(Long scheduleId, GeneticAlgorithmJobData data) {
        ...
        Map<Long, io.qusay.services.ga.geneticalgorithm.Module> moduleIndex = data.getModules().stream().collect(Collectors.toMap(io.qusay.services.ga.geneticalgorithm.Module::getId, module -> module));
